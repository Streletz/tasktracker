	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post())) {
			...
			if ($this->canSetTask($model)) {
				...
				$errorMessage = "Невозможно поставить задачу самому себе!";
				Yii::$app->session->setFlash('error_message', $errorMessage);
			}
		}
...
*/
	...
	protected function canSetTask($task)
	{
		$settingsModel=Settings::find()->one();
		if ($settingsModel!=null) {
			if ($settingsModel->can_set_myself_task) {
				return true;
			} else {
				return $task->creator_id != $task->worker_id;
			}
		}
		throw new ServerErrorHttpException("Ошибка настроек приложения.");
	}
